...
# Initialize the mode interface
hlci = cgi.CGI_PROPER(use_opds=False, use_fieldstop=True, quiet=True)
# hlci.set_dm1(dm1_flatmap)
# hlci.set_dm2(dm2_flatmap)


# create control and dark hole masks
...
dh_mask = iefc.create_dshaped_focal_plane_mask(xf, yf, dh_params).ravel()
...
control_mask = iefc.create_dshaped_focal_plane_mask(xf, yf, control_params).ravel()
...
fourier_modes, fx, fy = iefc.create_fourier_modes(xfp, control_mask.reshape((npsf,npsf)), Nact, circular_mask=False)
probe_modes = iefc.create_probe_poke_modes(hlci.Nact, 3*Nact//4, Nact//2, 3*Nact//4-1, Nact//2)
...
response_hdu.writeto(data_dir/'response-data'/'hlc_response_cube_annular.fits', overwrite=True)

calib_hdu = fits.PrimaryHDU(data=calibration_cube)
calib_hdu.writeto(data_dir/'calibration-data'/'hlc_calibration_cube_annular.fits', overwrite=True)
