    ...
    for i, mode in enumerate(modes):
        if dm_mask[i]==1:
            ...
            for amp in amps:
                ...
                psf = sysi.snap()
                response += amp*psf.wavefront/np.var(amps)
                sysi.add_dm1(-amp*mode)
...
def run_pwp(sysi, probes, jacobian, dark_mask, use_noise=False, display=False):
    ...
    for i,probe in enumerate(probes):
        for amp in amps:
            ...
            if isinstance(psf.intensity, cp.ndarray):
                psf = psf.intensity.get()
            else:
                psf = psf.intensity

            if use_noise:
                psf = sysi.add_noise(psf)
        ...
        if display:
            misc.myimshow3(Ip[i], In[i], Ip[i]-In[i],
                           ...
                           lognorm1=True, lognorm2=True, vmin1=1e-10, vmin2=1e-10)
        
    E_probes = np.zeros((probes.shape[0], 2*nmask))
    I_diff = np.zeros((probes.shape[0], nmask))
    for i in range(len(probes)):
        E_probe = jacobian.dot(np.array(probes[i])) # Use jacobian to model probe E-field at the focal plane
...
def run_efc_perfect(sysi, efc_matrix, dark_mask, efc_loop_gain=0.5, iterations=5, display=False):
    # This function is only for running EFC with a model
...
def run_efc_pwp(sysi, efc_matrix, jac, probes, dark_mask, efc_loop_gain=0.5, iterations=5, display=False):
    ...
    for i in range(iterations):
        ...
        psf = sysi.snap()
        if isinstance(psf.intensity, cp.ndarray):
            I_exact = psf.intensity.get()
        else:
            I_exact = psf.intensity
    ...
    clear_output()
    print('EFC/PWP completed in {:.3f} sec.'.format(time.time()-start))
