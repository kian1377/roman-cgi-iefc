...
probe_modes = iefcu.create_probe_poke_modes(Nact, Nact//2, (Nact+2)//2, (Nact-1)//2, Nact//2)
...
response_hdu.writeto(data_dir/'response-data'/'hlc_response_cube_annular_3to6_v4.fits', overwrite=True)

calib_hdu = fits.PrimaryHDU(data=calibration_cube)
calib_hdu.writeto(data_dir/'calibration-data'/'hlc_calibration_cube_annular_3to6_v4.fits', overwrite=True)
